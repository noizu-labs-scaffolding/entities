...
defmodule Noizu.Entity.Macros.Json do
  ...
  def expand_json_settings(json_list, fields) do
    ...
    ud = Enum.map(core, fn({k,v}) ->
      {k, Noizu.Entity.Macros.Json.merge_json_settings([v| (de[k] || [])] |> Enum.reverse() , :default, k)}
    end)

    Enum.map([{:default, ud} | (fp || [])], fn({k, v}) ->
      v3 = Enum.map(ud, fn({k2,[v2]}) ->
        [x] = Noizu.Entity.Macros.Json.merge_json_settings([v2| (v[k2] || [])] |> Enum.reverse(), k, k2)
        {k2, x}
      end)
           |> List.flatten()
  ...
  def extract_json__inner(json, field) do
    ...
    |> Enum.map(
         fn(entry) ->
           {t,s} = case entry do
             v when is_atom(v) -> {:default, Noizu.Entity.Macros.Json.exact_json__settings(v)}
             [for: k, set: v] -> {k,  Noizu.Entity.Macros.Json.exact_json__settings(v)}
             v = [{k,v2}] ->
               cond do
                 is_list(k) -> {k,  Noizu.Entity.Macros.Json.exact_json__settings(v2)}
                 k in [:omit, :include, :as] -> {:default, Noizu.Entity.Macros.Json.exact_json__settings(v)}
                 :else -> {k,  Noizu.Entity.Macros.Json.exact_json__settings(v2)}
               end
             v when is_list(v) -> {:default, Noizu.Entity.Macros.Json.exact_json__settings(v)}
             v ->
               {:error, [{:error, v}]}
           end
