defmodule Noizu.Entity.UUIDReference do
  ...
  def cast(v) do
    case v do
      nil -> {:ok, nil}
      <<_::binary-size(16)>> -> Noizu.Entity.UID.ref(v)
      <<_,_,_,_,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,_,_,_,_,_,_,_,_>> -> Noizu.Entity.UID.ref(v)
      %{__struct__: _} -> Noizu.EntityReference.Protocol.ref(v)
      _ -> :error
    end
  end
  ...
  def dump(<<_,_,_,_,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,_,_,_,_,_,_,_,_>> = v) do
    {:ok, v}
  end
  ...
  def load(v) do
    case v do
      ...
      <<_::binary-size(16)>> -> Noizu.Entity.UID.ref(v)
      <<_,_,_,_,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,_,_,_,_,_,_,_,_>> -> Noizu.Entity.UID.ref(v)
      v when is_struct(v) -> Noizu.EntityReference.Protocol.ref(v)
...
defmodule  Noizu.Entity.UUIDReference.TypeHelper do
  ...
  def field_from_record(
    ...
    case Map.get(record, as_name) do
      ...
      v = <<_::binary-size(16)>> -> Noizu.Entity.UID.ref(v) |> Noizu.EntityReference.Protocol.entity(context)
      v = <<_,_,_,_,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,?-,_,_,_,_,_,_,_,_,_,_,_,_>> -> Noizu.Entity.UID.ref(v) |> Noizu.EntityReference.Protocol.entity(context)
      v -> v
    end |> case do
             nil -> {:ok, {name, nil}}
...
defimpl Noizu.Entity.Store.Ecto.EntityProtocol, for: [Noizu.Entity.UUIDReference] do
  defdelegate persist(entity,type,settings,context,options), to: Noizu.Entity.UUIDReference.TypeHelper
  defdelegate as_record(entity,settings,context,options), to: Noizu.Entity.UUIDReference.TypeHelper
  defdelegate as_entity(entity,settings,context,options), to: Noizu.Entity.UUIDReference.TypeHelper
  defdelegate delete_record(entity,settings,context,options), to: Noizu.Entity.UUIDReference.TypeHelper
  defdelegate from_record(record,settings,context,options), to: Noizu.Entity.UUIDReference.TypeHelper
end

defimpl Noizu.Entity.Store.Ecto.Entity.FieldProtocol, for: [Noizu.Entity.UUIDReference] do
  defdelegate field_from_record(field, record, field_settings, persistence_settings, context, options), to: Noizu.Entity.UUIDReference.TypeHelper
  defdelegate field_as_record(field, field_settings, persistence_settings, context, options), to: Noizu.Entity.UUIDReference.TypeHelper
end
