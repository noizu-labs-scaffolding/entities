...
defmodule Noizu.Entity.DerivedField.TypeHelper do
  ...
  def field_as_record(
        field,
        Noizu.Entity.Meta.Field.field_settings(name: name, store: field_store, options: field_options) = field_settings,
    ...
    |> case do
         {:f5, sync_derived} -> sync_derived.(field, field_settings, persistence_settings, context, options)
         _ -> {:ok, {name, nil}}
       end
  ...
  def field_from_record(
        field_stub,
        record,
        field_settings = Noizu.Entity.Meta.Field.field_settings(name: name, store: field_store, options: field_options),
        persistence_settings = Noizu.Entity.Meta.Persistence.persistence_settings(store: store, table: table),
        context,
        options
      ) do
    ...
    |> case do
         ...
         {:f6, sync_derived} ->
           case sync_derived.(field_stub, record, field_settings, persistence_settings, context, options) do
             {:ok, v} -> {:ok, {name, v}}
             _ -> nil
           end
         _ ->
...
defimpl Noizu.Entity.Store.Ecto.EntityProtocol, for: [Noizu.Entity.DerivedField] do
  defdelegate persist(entity,type,settings,context,options), to: Noizu.Entity.DerivedField.TypeHelper
  defdelegate as_record(entity,settings,context,options), to: Noizu.Entity.DerivedField.TypeHelper
  defdelegate as_entity(entity,settings,context,options), to: Noizu.Entity.DerivedField.TypeHelper
  defdelegate delete_record(entity,settings,context,options), to: Noizu.Entity.DerivedField.TypeHelper
  defdelegate from_record(record,settings,context,options), to: Noizu.Entity.DerivedField.TypeHelper
end

defimpl Noizu.Entity.Store.Ecto.Entity.FieldProtocol, for: [Noizu.Entity.DerivedField] do
  defdelegate field_from_record(field, record, field_settings, persistence_settings, context, options), to: Noizu.Entity.DerivedField.TypeHelper
  defdelegate field_as_record(field, field_settings, persistence_settings, context, options), to: Noizu.Entity.DerivedField.TypeHelper
end
