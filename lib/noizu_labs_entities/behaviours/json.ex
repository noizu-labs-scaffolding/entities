...
defimpl Noizu.Entity.Json.Protocol, for: [Any] do
  ...
  def prep(%{__struct__: m} = term, settings, context, options) do
    with acl_config <- Noizu.Entity.Meta.acl(m),
         {:ok, restricted} <- Noizu.Entity.ACL.Protocol.restrict(:read, term, acl_config, context, options) do
      Enum.map(settings,
        fn ({field, field_settings}) ->
          with {:ok, v} <- (restricted
...
if Code.ensure_loaded?(Poison) do
  defmodule Noizu.Entity.Json.DefaultHandler do
    ...
    rescue e in Noizu.Entity.Json.Exception -> {:error, e.details}
    end
    def encode!(%{__struct__: m} = term, context, options) do
      settings = Noizu.Entity.Meta.json(m, get_format(term, context, options))
      term
  ...
  defmodule  Noizu.Entity.JsonBehaviour do
    @handler Application.compile_env(:noizu_labs_entities, :json_handler, Noizu.Entity.Json.DefaultHandler)
    ...
    @callback encode(entity :: any, json_settings :: any, context :: any, opts :: any) :: {:ok, any} | {:error, any}
